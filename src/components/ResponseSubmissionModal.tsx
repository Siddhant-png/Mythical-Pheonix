...
export const ResponseSubmissionModal: React.FC<ResponseSubmissionModalProps> = ({ mode, problem, onClose, onSubmitOutcome, onSubmitProposal }) => {
  ...
  const [patentIpId, setPatentIpId] = useState('');
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    onSubmitProposal({
      ...
      proposalTitle,
      organizationName,
      patentIpId: patentIpId.trim() || undefined,
      executiveSummary,
  ...
  const inputClass = 'w-full rounded-md border border-slate-200 px-3 py-2 text-xs text-slate-800 outline-none focus:border-slate-500 focus:ring-1 focus:ring-slate-400';
  const textAreaClass = `${inputClass} resize-y`;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-950/70 p-3 sm:p-6">
      <div className="my-4 w-full max-w-2xl overflow-hidden rounded-lg border border-slate-200 bg-white shadow-xl">
        <div className="flex items-start justify-between bg-slate-900 p-4 text-white">
          <div>
            ...
            <h2 className="mt-0.5 text-lg font-extrabold">{mode === 'outcome' ? 'Post Your Outcome' : 'Submit Solution Proposal'}</h2>
            <p className="mt-0.5 max-w-xl text-xs text-slate-300">For: {problem.title}</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="max-h-[78vh] space-y-3.5 overflow-y-auto p-4 sm:p-5">
          {mode === 'outcome' ? (
            <>
              ...
              <label className="block text-xs font-bold text-slate-800">Short Description<textarea required rows={3} value={outcomeDescription} onChange={event => setOutcomeDescription(event.target.value)} placeholder="Our prototype detects potholes from road footage and generates location-based reports." className={`${textAreaClass} mt-1`} /></label>
              <label className="block text-xs font-bold text-slate-800">Demo Video<div className="mt-1 flex items-center gap-3 rounded-md border border-dashed border-slate-300 p-2.5"><Video className="h-4 w-4 text-slate-600" /><input type="file" accept="video/mp4,video/webm,video/quicktime" onChange={event => setVideoName(event.target.files?.[0]?.name || '')} className="min-w-0 flex-1 text-xs text-slate-500" />{videoName && <span className="truncate text-xs font-semibold text-slate-700">{videoName}</span>}</div></label>
            ...
            <>
              ...
              <Field 
                label="Patent / Intellectual Property (IP) ID (Optional)" 
                value={patentIpId} 
                onChange={setPatentIpId} 
                placeholder="e.g. IN-PAT-2024-88910 or IPO-98214" 
              ...
              <TextField label="Proposed Solution" value={proposedSolution} onChange={setProposedSolution} placeholder="Describe your proposed solution in detail and how it addresses the government problem." rows={5} inputClass={textAreaClass} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <Field label="Proposed Implementation Timeline" value={timeline} onChange={setTimeline} placeholder="e.g. 6 weeks" inputClass={inputClass} />
              ...
              <label className="block text-xs font-bold text-slate-800">Supporting Documents<div className="mt-1 flex items-center gap-3 rounded-md border border-dashed border-slate-300 p-2.5"><Upload className="h-4 w-4 text-slate-600" /><input type="file" multiple onChange={event => setSupportingDocuments(Array.from(event.target.files || []).map(file => file.name))} className="min-w-0 flex-1 text-xs text-slate-500" />{supportingDocuments.length > 0 && <span className="text-xs font-semibold text-slate-700">{supportingDocuments.length} selected</span>}</div></label>
            </>
          )}
          <div className="flex justify-end gap-2 border-t border-slate-100 pt-3"><button type="button" onClick={onClose} className="px-3 py-1.5 text-xs font-semibold text-slate-600 hover:text-slate-900">Cancel</button><button type="submit" className="inline-flex items-center gap-1.5 rounded-md bg-slate-900 px-4 py-2 text-xs font-bold text-white shadow-sm transition hover:bg-slate-800"><FileText className="h-3.5 w-3.5" />{mode === 'outcome' ? 'Post Outcome' : 'Submit Proposal'}</button></div>
        </form>
      </div>
    </div>
  );
...
const Field = ({ label, value, onChange, placeholder, required, inputClass }: { label: string; value: string; onChange: (value: string) => void; placeholder: string; required?: boolean; inputClass: string }) => <label className="block text-xs font-bold text-slate-800">{label}<input required={required} value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder} className={`${inputClass} mt-1`} /></label>;

const TextField = ({ label, value, onChange, placeholder, rows = 3, inputClass }: { label: string; value: string; onChange: (value: string) => void; placeholder: string; rows?: number; inputClass: string }) => <label className="block text-xs font-bold text-slate-800">{label}<textarea required value={value} onChange={event => onChange(event.target.value)} placeholder={placeholder} rows={rows} className={`${inputClass} mt-1`} /></label>;