...
export const SelfAccountProfile: React.FC<SelfAccountProfileProps> = ({
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-5 font-body pb-12 select-none">
      
      {/* Top Banner & Profile Header */}
      <div className="bg-white rounded-lg border border-slate-200 shadow-xs overflow-hidden">
        {/* Decorative Cover Header */}
        <div className="h-36 bg-slate-900 relative p-6 flex items-start justify-between">
          <div className="relative z-10 flex items-center space-x-2 bg-white/10 px-3 py-1 rounded-md text-xs text-slate-200 font-bold border border-white/10">
          ...
          <div className="relative z-10 flex items-center space-x-2">
            <button
              ...
              className="px-3 py-1.5 bg-white/10 hover:bg-white/20 text-white rounded-md text-xs font-bold transition border border-white/10"
            >
              Export Passport PDF
            </button>
          </div>
        ...
        <div className="px-6 pb-6 pt-0 relative">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-4">
            <div className="flex flex-col sm:flex-row sm:items-end space-y-3 sm:space-y-0 sm:space-x-5">
              ...
              <div className="relative group -mt-12">
                <div className="w-24 h-24 rounded-full bg-slate-900 text-white font-black text-2xl flex items-center justify-center border-4 border-white shadow-md">
                  {userInitials}
                </div>
              </div>
            ...
            <div className="flex items-center space-x-2 shrink-0">
              <span className="text-xs bg-emerald-100 text-emerald-900 font-extrabold px-3 py-1.5 rounded-md border border-emerald-300">
                {currentUser?.verificationBadge || 'DPIIT Certified Startup (DIPP-MH-98442)'}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-4 border-t border-slate-100 text-xs">
            ...
            <div className="bg-slate-50 p-3 rounded-md border border-slate-200">
              ...
              <span className="font-extrabold text-slate-900 text-xs">{pilots.length} Running</span>
            </div>
            <div className="bg-slate-50 p-3 rounded-md border border-slate-200">
              <span className="text-slate-400 block text-[10px] font-bold uppercase">State Escrow Balance</span>
              <span className="font-extrabold text-slate-900 text-xs">₹38.0 Lakhs Locked</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2 border-b border-slate-200 pb-2">
        ...
        >
          Identity & Government Credentials
        </button>
        ...
        >
          My Tender Applications ({myBids.length})
        </button>
        ...
        >
          Active Testbeds & KPI Scorecards
        </button>
        ...
        >
          Consortium NDAs ({collaborations.length})
        </button>
          ...
          <div className="lg:col-span-2 space-y-5">
            ...
            <div className="bg-white rounded-md border border-slate-200 p-5 space-y-4">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                ...
                <span className="text-[10px] bg-emerald-100 text-emerald-900 font-extrabold px-2.5 py-1 rounded">
                  100% Compliant
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs">
                ...
                <div className="bg-slate-50 p-3 rounded-md border border-slate-200">
                  ...
                  <span className="font-mono font-bold text-slate-900 text-sm block mt-0.5">{currentStartup.dpiitCertNo}</span>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {currentStartup.techDomains.map((domain, i) => (
                    <span key={i} className="bg-slate-100 text-slate-900 border border-slate-200 font-bold text-xs px-2.5 py-1 rounded-md">
                      {domain}
                    </span>
                  ))}
                </div>
          ...
          <div className="space-y-5">
            <div className="bg-white rounded-md border border-slate-200 p-5 space-y-3">
              ...
              <div className="space-y-2 text-xs">
                <div className="flex items-center justify-between p-2.5 rounded-md bg-slate-50 border border-slate-200">
          ...
          <div className="bg-white rounded-md border border-slate-200 p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="px-3 py-1.5 bg-slate-900 text-white font-bold text-xs rounded-md hover:bg-slate-800 transition"
              >
            ...
            <div className="space-y-3">
              {myBids.map((bid) => (
                <div key={bid.id} className="border border-slate-200 rounded-md p-4 bg-slate-50 space-y-3">
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 pb-3">
                    <div>
                      <div className="flex items-center space-x-2">
                        <span className="text-[10px] bg-slate-200 text-slate-900 font-extrabold px-2 py-0.5 rounded uppercase">
                          {bid.sector}
                        </span>
                    ...
                    <div className="text-right bg-white p-2.5 rounded-md border border-slate-200 shrink-0">
      ...
      {activeSection === 'sandboxes' && (
        <div className="space-y-4">
          <div className="bg-white rounded-md border border-slate-200 p-5 space-y-4">
            ...
            <div className="space-y-4">
              {pilots.map((pilot) => (
                <div key={pilot.id} className="border border-slate-200 rounded-md p-4 bg-white space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-xl font-black text-emerald-700">{pilot.aggregateScore}/100</span>
                    </div>
                  </div>

                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                    {pilot.scorecards.map((sc, i) => (
                      <div key={i} className="bg-slate-50 p-2.5 rounded-md border border-slate-200">
                        <span className="font-bold text-slate-800 block text-[11px]">{sc.metric}</span>
                        <div className="text-xs text-slate-900 font-bold mt-1">Target: {sc.target}</div>
          ...
          <div className="bg-white rounded-md border border-slate-200 p-5 space-y-4">
            ...
            <div className="space-y-3">
              {collaborations.map((collab) => (
                <div key={collab.id} className="border border-slate-200 rounded-md p-4 bg-slate-50 flex items-center justify-between gap-4 text-xs">
                  <div>
                    <div className="flex items-center space-x-2">
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 bg-white border border-slate-200 rounded-md font-bold text-slate-700 hover:bg-slate-100 transition shrink-0"
                  >
                    View NDA Document
                  </button>
                </div>
      ...
      {activeSection === 'settings' && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          ...
          <div className="bg-white rounded-md border border-slate-200 p-5 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <button
                ...
                className={`p-3 rounded-md border text-left font-bold transition ${
                  userRole === 'startup'
          ...
          <div className="bg-white rounded-md border border-slate-200 p-5 space-y-4">
            ...
            <div className="bg-slate-900 text-white p-4 rounded-md space-y-2 font-mono text-xs">
