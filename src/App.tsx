...
import { ProposedSolutionsView } from './views/ProposedSolutionsView';
...
export function App() {
  ...
  const [proposals, setProposals] = useState<SolutionProposal[]>(INITIAL_PROPOSALS);
  ...
  const handleUpdateProposalStatus = (proposalId: string, newStatus: ProposalStatus) => {
    setProposals(previous => previous.map(p => p.id === proposalId ? { ...p, status: newStatus } : p));
    showToast(`Proposal status updated to ${newStatus.replace(/_/g, ' ').toLowerCase()}.`);
  };
